...
class SdlHardware
{
    ...
    static int flags;
    static int colors;
    static DateTime lastPause;


    public static void Init(short w, short h, int bitsPerPixel, bool fullScreen)
    {
        width = w;
        height = h;
        colors = bitsPerPixel;
    ...
    public static void DrawHiddenImage(Sprite s)
    {
    ...
    public static bool IsKeyPressed(int c)
    {
        bool pressed = false;
        Sdl.SDL_PumpEvents();
    ...
    public static void Pause(int millisecondsToWait)
    {
        DateTime currentMoment = DateTime.Now;
        TimeSpan timeElapsed = currentMoment - lastPause;
        int elapsedMilliseconds = (int)timeElapsed.TotalMilliseconds;
        if (elapsedMilliseconds < millisecondsToWait)
            Thread.Sleep(millisecondsToWait - elapsedMilliseconds);
        lastPause = DateTime.Now;
    }
    ...
    public static bool JoystickMoved(out int posX, out int posY)
    {
        ...
        posX = Sdl.SDL_JoystickGetAxis(joystick, 0);  // Let's read values (up to 32768)
        posY = Sdl.SDL_JoystickGetAxis(joystick, 1);
        // And let's normalize to -1, +1 or 0
    ...
    public static bool MouseClicked()
    {
        ...
        // Now let's see it really there is a click
    ...
    public static void FilledRectangleRGBA(short x1, short y1, short x2, short y2,
      byte r, byte g, byte b, byte a)
    {
        SdlGfx.boxRGBA(hiddenScreen,
          x1, y1, x2, y2, r, g, b, a);
    }
    ...
    public static int KEY_F1 = Sdl.SDLK_F1;
